...
function _border(...args) {
    ...
    log.styles.push(`${prop}: ${vals.join(' ')};`)
    return log
}

function _bg(bgcolor) {
...
function _font(...args) {
    ...
    args.forEach(arg => {
        if(style.includes(arg)) {
            log.styles.push(`font-style: ${arg};`)
        } else if(weight.includes(arg)) {
            log.styles.push(`font-weight: ${arg};`)
        } else if(decoration.includes(arg)) {
            log.styles.push(`text-decoration: ${arg};`)
        } else if(typeof arg === 'number') {
            log.styles.push(`font-size: ${arg}px;`)
        } else if(CSS.supports('color', arg)) {
            log.styles.push(`color: ${arg};`)
        }
    })
    return this
}
...
for (const key in colors) {
    Object.defineProperty(log, key, {
        get() {
            log.styles.push(`color: ${colors[key]};`)
            return log
        },
    })

    const bgKey = `bg${key.at(0).toUpperCase()}${key.slice(1)}`
    Object.defineProperty(log, bgKey, {
        get() {
            log.styles.push(`background-color: ${colors[key]};`)
            return log
        },
    })
}
